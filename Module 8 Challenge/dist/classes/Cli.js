...
class Cli {
    ...
    findVehicleToTow() {
        if (this.vehicles.length < 2) {
            console.log('There are no vehicles to tow');
            return;
        }
        inquirer
            .prompt([
            ...
            const truck = this.vehicles.find((vehicle) => vehicle.vin === this.selectedVehicleVin);
            if (!truck) {
                console.log('Truck not found');
                this.performActions();
                return;
            }
            console.log('Attempting to tow vehicle');
            truck.tow(vehicleToTow);
            this.performActions();
        })
            .catch((error) => {
            console.log('Error finding vehicle to tow', error);
            this.performActions;
        });
    }
